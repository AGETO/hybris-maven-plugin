...
import com.divae.ageto.hybris.install.task.*;
...
public enum InstallStrategy {
    ...
    static List<InstallTask> getInstallTasks(final TaskContext taskContext) {
        ...
        return Arrays.asList( //
                ...
                new ExecuteMavenTask("", new String[] {
                        "install:install-file",
                        String.format("-Dfile=%s", new File(taskContext.getHybrisDirectory(),
                                "bin/platform/bootstrap/bin/ybootstrap.jar")), "-DgroupId=de.hybris", "-DartifactId=bootstrap",
                        String.format("-Dversion=%s", taskContext.getHybrisVersion().getVersion()), "-Dpackaging=jar" }, true), //
